...
from aws_ops_alpha.constants import DEVOPS
from aws_ops_alpha.paths import dir_project_root
from aws_ops_alpha.runtime import runtime
from aws_ops_alpha.environment import BaseWorkloadEnvEnum, detect_current_env
...
class MyConfig(Config):
    ...
    def get_current_env(cls) -> str:  # pragma: no cover
        raise detect_current_env(runtime, MyEnvEnum)

    @cached_property
    def sbx(self) -> Env:  # pragma: no cover
        return self.get_env(env_name=MyEnvEnum.sbx)

    @cached_property
    def prd(self) -> Env:  # pragma: no cover
        return self.get_env(env_name=MyEnvEnum.prd)
...
boto_ses_factory = BotoSesFactory(
    ...
    env_to_profile_mapper={
        ...
        MyEnvEnum.sbx.value: "bmt_app_dev_us_east_1",
        MyEnvEnum.prd.value: "bmt_app_prod_us_east_1",
    },
    default_app_env_name=MyEnvEnum.sbx.value,
)
